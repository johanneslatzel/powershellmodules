...
function Add-PRTGChannel() {
	...
	param(
		...
		[Parameter(Position=4, Mandatory=$false)][PRTGChannelLimit]$limit = $null,
		[Parameter(Position=5, Mandatory=$false)][string]$value_lookup_id = $null,
		[Parameter(Position=5, Mandatory=$false)][switch]$is_float = $false,
		[Parameter(Position=6, Mandatory=$false)][string]$custom_unit = "units"
    )
	Process {
...
function Set-PRTGSensorText() {
	...
	param(  
		[Parameter(Position=0, Mandatory=$true, ValueFromPipeline=$true)][PRTGSensor]$sensor,
		[Parameter(Position=1, Mandatory=$true)][string]$text
    )
	Process {
		$sensor.setText($text)
        return $sensor
...
function New-PRTGChannelLimit() {
	[cmdletbinding()]
	param(
		[Parameter(Position=0, Mandatory=$false)][float]$warning_min,
		[Parameter(Position=1, Mandatory=$false)][float]$warning_max,
		[Parameter(Position=2, Mandatory=$false)][float]$error_min,
		[Parameter(Position=3, Mandatory=$false)][float]$error_max,
		[Parameter(Position=4, Mandatory=$false)][switch]$DisableMin,
		[Parameter(Position=5, Mandatory=$false)][switch]$DisableMax,
		[Parameter(Position=6, Mandatory=$false)][switch]$DisableWarningMin,
		[Parameter(Position=7, Mandatory=$false)][switch]$DisableWarningMax,
		[Parameter(Position=8, Mandatory=$false)][switch]$DisableErrorMin,
		[Parameter(Position=9, Mandatory=$false)][switch]$DisableErrorMax
    )
	Process {
